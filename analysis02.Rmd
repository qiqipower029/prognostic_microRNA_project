...
for (i in 1:60) {
  model = glm(formula = EV.grade.CTMean$`Adverse Pathology`~ EV.NormFinder.new2[,i+1], family = "binomial")
  result = summary(model)
...
# serum geNorm
fullmodel.serum.geNorm = glm(formula = serum.all.original$`Biopsy Grade Group`~ serum.globalCTMean.new$`hsa-miR-1246`+ serum.globalCTMean.new$`hsa-miR-214-3p`+ serum.globalCTMean.new$`hsa-miR-301a-3p` + )
