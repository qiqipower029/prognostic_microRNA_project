...
# Display result
grade.serum.result = grade.serum.result[order(grade.serum.result$p.value),]
...
for (i in 1:61) {
  model = glm(formula = EV.grade.CTMean$`Biopsy Grade Group`~ EV.NormFinder.new[,i+1])
  result = summary(model)
...
# Collect the results
serum.GLM.result = write.xlsx(list(grade.serum.result, grade.serum.CTMean.result, grade.serum.geNorm.result, grade.serum.NormFinder.result), file = "serumGLMresults.xlsx")
EV.GLM.result = write.xlsx(list(grade.EV.result, grade.EV.CTMean.result, grade.EV.geNorm.result, grade.EV.NormFinder.result), file = "EVGLMresults.xlsx")
```
